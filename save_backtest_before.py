"""
Sauvegarde complete du backtest AVANT re-download.
On garde tous les trades pour comparaison apres.
"""
...
for day in trading_days:
    ...
    def add(sn, d, e, pi):
        ...
        S.setdefault(sn, []).append({'date':str(day),'dir':d,'pnl_oz':pnl-get_sp(day),'entry':e,'exit':ex,'bars':b,'atr':atr,'ei':pi})
...
for t in combined:
    ...
    acc.append(t); al.append((t['ei']+t['bars'], t['dir']))

n = len(acc)
cap = 1000.0; peak = cap; max_dd = 0; gp = 0; gl = 0; wins = 0
for t in acc:
    pnl = t['pnl_oz'] * (cap * 0.01) / (SL * t['atr'])
    cap += pnl
...
result = {
    'label': 'AVANT re-download (bougies open=prev_close)',
    'n_trades': n,
    'capital_final': cap,
    'rendement_pct': (cap-1000)/1000*100,
    'max_dd_pct': max_dd*100,
    'pf': gp/(gl+0.01),
    'wr': wins/n*100,
    'strats': {sn: len(S[sn]) for sn in sorted(S.keys())},
}

# Sauvegarder aussi les 10 derniers trades par strat pour comparaison
result['sample_trades'] = {}
for sn in sorted(S.keys()):
    result['sample_trades'][sn] = S[sn][-5:] if len(S[sn]) >= 5 else S[sn]

print(f"\nRESULTAT AVANT RE-DOWNLOAD:")
print(f"  Trades: {n}")
print(f"  Capital: ${cap:,.0f}")
print(f"  Rendement: {(cap-1000)/1000*100:+.0f}%")
print(f"  DD: {max_dd*100:.1f}%")
print(f"  PF: {gp/(gl+0.01):.2f}")
print(f"  WR: {wins/n*100:.0f}%")
print(f"\n  Trades par strat:")
for sn in sorted(S.keys()):
    print(f"    {sn}: {len(S[sn])}")

with open('backtest_before_redownload.json', 'w') as f:
    json.dump(result, f, indent=2, default=str)
print("\nSauvegarde dans backtest_before_redownload.json")
