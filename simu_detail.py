"""
Simulation detaillee mois par mois — AA+D+E+F+H+NY6+NY16+NY17+O
Config: TRAIL SL=1.0 ACT=0.5 TRAIL=0.75 no timeout (sur CLOSE)
Usage: python simu_detail.py [capital] [risk%]
  Ex: python simu_detail.py 100000 0.05
"""
...
risk = float(sys.argv[2]) / 100 if len(sys.argv) > 2 else 0.0005
...
print("="*140)
print(f"  9 strats: AA+D+E+F+H+NY6+NY16+NY17+O -- ${capital:,.0f}, Risk {risk*100:.2f}%")
print(f"  Config: TRAIL SL={SL} ACT={ACT} TRAIL={TRAIL} MX={MX} (sur CLOSE)")
print("="*140)
...
for mo in sorted(months.keys()):
    ...
    print(f"    {'Strat':>5s} {'Trades':>7s} {'W':>3s} {'L':>3s} {'WR':>5s} {'PnL $':>14s}")
    for sn in sorted(m['strats'].keys()):
        ss = m['strats'][sn]
        wr_s = ss['w']/ss['n']*100 if ss['n'] else 0
        print(f"    {sn:>5s} {ss['n']:7d} {ss['w']:3d} {ss['n']-ss['w']:3d} {wr_s:4.0f}% ${ss['pnl']:+14,.2f}")
