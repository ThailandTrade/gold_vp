"""
Optimisation v7 — backtest SANS look-ahead.
Itere bougie par bougie, exactement comme le live.
A chaque bougie, ne voit que le passe.
"""
...
# Pre-calcul daily direction pour Z
daily_data = {}
...
# ══════════════════════════════════════════════════════════════
# COLLECTE BOUGIE PAR BOUGIE — exactement comme le live
# ══════════════════════════════════════════════════════════════
print("Collecte bougie par bougie (no look-ahead)...", flush=True)
S = {}  # {strat: [trades]}

prev_candle_date = None
trig = {}   # cooldown par jour
ibs = {}    # IB levels + P ORB levels
day_atr = None
...
for ci in range(n_candles):
    ...
    # Visible = toutes les bougies jusqu'a celle-ci (incluse)
    # Pour la perf, on ne prend pas .tail(1500), on filtre par date
    # Les strats n'ont besoin que des bougies du jour courant + veille pour tc

    def add(sn, d, e):
        pi = ci  # candle index = position dans le dataframe
        b, ex = sim_trail(candles, pi, e, d, SL, atr, 24, ACT, TRAIL)
        pnl = (ex-e) if d=='long' else (e-ex)
        S.setdefault(sn, []).append({
            'date':today,'dir':d,'sl_atr':SL,'pnl_oz':pnl-get_sp(today),
            'atr':atr,'ei':pi,'xi':pi+b
        })

    # Bougies sessions du jour courant (jusqu'a la bougie courante incluse)
    day_start = pd.Timestamp(today.year, today.month, today.day, 0, 0, tz='UTC')
    tok_end = pd.Timestamp(today.year, today.month, today.day, 6, 0, tz='UTC')
    lon_start = pd.Timestamp(today.year, today.month, today.day, 8, 0, tz='UTC')
    lon_end = pd.Timestamp(today.year, today.month, today.day, 14, 30, tz='UTC')
    ny_start = pd.Timestamp(today.year, today.month, today.day, 14, 30, tz='UTC')
    ny_end = pd.Timestamp(today.year, today.month, today.day, 21, 30, tz='UTC')

    # Bougies visibles dans chaque session (seulement celles <= bougie courante)
    mask_today = (candles['ts_dt'] >= day_start) & (candles['ts_dt'] <= candle_time)
    today_candles = candles[mask_today]

    tok = today_candles[today_candles['ts_dt'] < tok_end]
    lon = today_candles[(today_candles['ts_dt'] >= lon_start) & (today_candles['ts_dt'] < lon_end)]
    ny = today_candles[(today_candles['ts_dt'] >= ny_start) & (today_candles['ts_dt'] < ny_end)]

    # ── A: IB Tokyo 1h break UP ──
    if 'A_done' not in ibs and hour >= 1.0:
        ib = today_candles[(today_candles['ts_dt'] >= day_start) & (today_candles['ts_dt'] < day_start + pd.Timedelta(hours=1))]
        if len(ib) >= 12 and len(tok) >= 18:
            ibs['A_high'] = float(ib['high'].max()); ibs['A_done'] = True
    if 'A_high' in ibs and 'A' not in trig and 1.0 <= hour < 6.0:
        if row['close'] > ibs['A_high']:
            add('A', 'long', row['close']); trig['A'] = True

    # ── C: FADE Tokyo → London ──
    if 8.0 <= hour < 8.1 and 'C' not in trig:
        if len(tok) >= 10:
            m = (tok.iloc[-1]['close'] - tok.iloc[0]['open']) / atr
            if abs(m) >= 1.0:
                add('C', 'short' if m > 0 else 'long', row['open']); trig['C'] = True

    # ── D: GAP Tokyo-London ──
    if 8.0 <= hour < 8.1 and 'D' not in trig:
        tc = candles[(candles['ts_dt'] < tok_end) & (candles['ts_dt'] <= candle_time)]
        if len(tc) >= 5:
            gap = (row['open'] - tc.iloc[-1]['close']) / atr
            if abs(gap) >= 0.5:
                add('D', 'long' if gap > 0 else 'short', row['open']); trig['D'] = True

    # ── E: KZ London 8-10h fade ──
    if 10.0 <= hour < 10.1 and 'E' not in trig:
        kz = today_candles[(today_candles['ts_dt'] >= lon_start) & (today_candles['ts_dt'] < pd.Timestamp(today.year,today.month,today.day,10,0,tz='UTC'))]
        if len(kz) >= 20:
            m = (kz.iloc[-1]['close'] - kz.iloc[0]['open']) / atr
            if abs(m) >= 0.5:
                add('E', 'short' if m > 0 else 'long', row['open']); trig['E'] = True
    ...
    if 14.5 <= hour < 14.6 and 'G' not in trig:
        ...
        if abs(body) >= 0.3 * atr:
            add('G', 'long' if body > 0 else 'short', row['close']); trig['G'] = True

    # ── H: TOKEND 3b ──
    if 8.0 <= hour < 8.1 and 'H' not in trig:
        if len(tok) >= 9:
            l3 = tok.iloc[-3:]
            m = (l3.iloc[-1]['close'] - l3.iloc[0]['open']) / atr
            if abs(m) >= 1.0:
                add('H', 'long' if m > 0 else 'short', row['open']); trig['H'] = True

    # ── I: FADENY 1h ──
    if 15.5 <= hour < 15.6 and 'I' not in trig:
        ny1 = today_candles[(today_candles['ts_dt'] >= ny_start) & (today_candles['ts_dt'] < pd.Timestamp(today.year,today.month,today.day,15,30,tz='UTC'))]
        if len(ny1) >= 10:
            m = (ny1.iloc[-1]['close'] - ny1.iloc[0]['open']) / atr
            if abs(m) >= 1.0:
                add('I', 'short' if m > 0 else 'long', row['open']); trig['I'] = True

    # ── J: LON 1st candle ──
    if 8.0 <= hour < 8.1 and 'J' not in trig:
        body = row['close'] - row['open']
        if abs(body) >= 0.3 * atr:
            add('J', 'long' if body > 0 else 'short', row['close']); trig['J'] = True

    # ── O: BigCandle Tokyo ──
    if 0.0 <= hour < 6.0 and 'O' not in trig:
        body = row['close'] - row['open']
        if abs(body) >= 1.0 * atr:
            add('O', 'long' if body > 0 else 'short', row['close']); trig['O'] = True

    # ── P: ORB NY 30min ──
    if 15.0 <= hour < 21.5 and 'P' not in trig:
        if 'P_high' not in ibs:
            orb = today_candles[(today_candles['ts_dt'] >= ny_start) & (today_candles['ts_dt'] < pd.Timestamp(today.year,today.month,today.day,15,0,tz='UTC'))]
            if len(orb) >= 6:
                ibs['P_high'] = float(orb['high'].max()); ibs['P_low'] = float(orb['low'].min())
        if 'P_high' in ibs:
            if row['close'] > ibs['P_high']:
                add('P', 'long', row['close']); trig['P'] = True
            elif row['close'] < ibs['P_low']:
                add('P', 'short', row['close']); trig['P'] = True
    ...
    if 8.0 <= hour < 14.5 and 'Q' not in trig and len(lon) >= 2:
        ...
        if prev_b['close']<prev_b['open'] and cur_b['close']>cur_b['open'] and cur_b['open']<=prev_b['close'] and cur_b['close']>=prev_b['open'] and abs(cur_b['close']-cur_b['open'])>=0.3*atr:
            add('Q', 'long', cur_b['close']); trig['Q'] = True
            ...
            add('Q', 'short', cur_b['close']); trig['Q'] = True

    # ── R: 3soldiers Tokyo cont ──
    ...
    if 0.0 <= hour < 6.0 and 'V' not in trig and len(tok) >= 7:
        ...
        if n_bull >= 5:
            add('V', 'long', row['close']); trig['V'] = True
        elif n_bull <= 1:
            add('V', 'short', row['close']); trig['V'] = True

    # ── Z: 3 jours consecutifs → reversal ──
...
print(f"\n  Collecte terminee en {elapsed:.0f}s", flush=True)

# Stats individuelles
print("\n" + "="*80)
print("STATS INDIVIDUELLES (18 strats — NO LOOK-AHEAD)")
print("="*80)
