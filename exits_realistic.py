"""
Exits realistes: SL >= 0.5 ATR, trail >= 0.3 ATR
Trailing pessimiste uniquement + SL/TP fixe + time-based
"""
...
# Configs REALISTES uniquement
configs = []
# SL/TP fixe
for sl in [0.5, 0.75, 1.0, 1.5]:
    for tp_r in [1.0, 1.5, 2.0, 3.0, 4.0]:
        tp = sl * tp_r
        for mx in [12, 24]:
            configs.append((f"SLTP s{sl} {tp_r}R T{mx}", lambda ci,d,e,atr,day,_sl=sl,_tp=tp,_mx=mx: exit_sltp(candles,ci,e,d,_sl,_tp,atr,_mx)))
# Time-based
...
# Trailing pessimiste REALISTE: SL >= 0.5, trail >= 0.3
for sl in [0.5, 0.75, 1.0, 1.5]:
...
print(f"{len(configs)} configs realistes", flush=True)

STRATS = sorted(signals.keys())
print(f"\n{'='*100}")
print(f"TOP 3 par strat — REALISTE (SL>=0.5, trail>=0.3)")
print(f"{'='*100}")
...
for sn in STRATS:
    ...
    for config_name, exit_fn in configs:
        ...
        ok = f1>0 and f2>0
        results.append((config_name, len(pnls), wr, pf, pnls.mean(), ok))
    ...
    print(f"\n  {sn} ({len(sigs)} trades):")
    if valid:
        print(f"    {'Config':40s} {'n':>4s} {'WR':>5s} {'PF':>6s} {'Avg':>8s}")
        for config_name, n, wr, pf, avg, ok in valid[:3]:
            marker = " ***" if pf >= 1.3 else ""
            print(f"    {config_name:40s} {n:4d} {wr:4.0f}% {pf:6.2f} {avg:+8.3f}{marker}")
    else:
        print(f"    PAS DE CONFIG VIABLE (split OK, PF>=1.0)")
