"""
Exploration exits v3 — CONSERVATEUR MAX
Trailing pessimiste: apres update, re-check low vs nouveau stop.
Compare avec SL/TP fixe et time-based.
"""
...
# ── TRAILING PESSIMISTE: re-check low/high apres CHAQUE update ──
def exit_trail_pessimist(cdf, pos, entry, d, sl, atr, mx, act, trail):
    ...
    for j in range(1, mx+1):
        ...
        if d == 'long':
            ...
            # PESSIMISTE: re-check low ET close vs nouveau stop
            if b['low'] <= stop: return j, stop
...
# ── BE PESSIMISTE ──
def exit_be_pessimist(cdf, pos, entry, d, sl_atr, atr, mx, be_trigger):
    sl = entry - sl_atr*atr if d == 'long' else entry + sl_atr*atr
...
# ── CONFIGS: sans ambiguite + trailing/BE pessimiste ──
configs = []
# SL/TP fixe (ZERO ambiguite)
for sl in [0.3, 0.5, 0.75, 1.0, 1.25, 1.5]:
    for tp_r in [1.0, 1.5, 2.0, 3.0, 4.0, 5.0]:
        tp = sl * tp_r
        for mx in [12, 24, 48]:
            configs.append((f"SLTP s{sl} tp{tp_r}R T{mx}", lambda ci,d,e,atr,day,_sl=sl,_tp=tp,_mx=mx: exit_sltp(candles,ci,e,d,_sl,_tp,atr,_mx)))
# Time-based (ZERO ambiguite)
for sl in [0.3, 0.5, 0.75, 1.0, 1.5]:
    for t in [3, 4, 6, 8, 12, 18, 24, 36, 48]:
        configs.append((f"TIME s{sl} T{t}b", lambda ci,d,e,atr,day,_sl=sl,_t=t: exit_time(candles,ci,e,d,_sl,atr,_t)))
# Trailing PESSIMISTE (re-check low apres update)
for sl in [0.3, 0.5, 0.75, 1.0]:
    for act in [0.2, 0.3, 0.5, 0.75, 1.0]:
        for trail in [0.15, 0.2, 0.3, 0.5]:
            if trail >= sl: continue
            for mx in [12, 24, 48]:
                configs.append((f"TRp s{sl} a{act} t{trail} T{mx}", lambda ci,d,e,atr,day,_sl=sl,_a=act,_t=trail,_mx=mx: exit_trail_pessimist(candles,ci,e,d,_sl,atr,_mx,_a,_t)))
# BE PESSIMISTE
for sl in [0.5, 0.75, 1.0]:
    for be in [0.3, 0.5, 0.75, 1.0]:
        for mx in [12, 24, 48]:
            configs.append((f"BEp s{sl} be{be} T{mx}", lambda ci,d,e,atr,day,_sl=sl,_be=be,_mx=mx: exit_be_pessimist(candles,ci,e,d,_sl,atr,_mx,_be)))

print(f"{len(configs)} configs", flush=True)

STRATS = sorted(signals.keys())
print(f"\n{'='*100}")
print(f"TOP 5 par strat — CONSERVATEUR MAX ({len(configs)} configs)")
print(f"{'='*100}")
...
for sn in STRATS:
    ...
    for config_name, exit_fn in configs:
        ...
        ok = f1>0 and f2>0
        results.append((config_name, len(pnls), wr, pf, pnls.mean(), ok, config_name[:4]))

    valid = [r for r in results if r[5] and r[3] >= 1.0]
    valid.sort(key=lambda x: -x[3])

    print(f"\n  {sn} ({len(sigs)} trades):")
    print(f"    {'Config':40s} {'Type':5s} {'n':>4s} {'WR':>5s} {'PF':>6s} {'Avg':>8s}")
    for config_name, n, wr, pf, avg, ok, ctype in valid[:5]:
        marker = " ***" if pf >= 1.3 else ""
        print(f"    {config_name:40s} {ctype:5s} {n:4d} {wr:4.0f}% {pf:6.2f} {avg:+8.3f}{marker}")
    if not valid:
        print(f"    AUCUNE config viable")
